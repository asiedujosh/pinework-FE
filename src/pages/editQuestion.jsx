...
import EditTextArea from "../component/editTextAreaField"
import SliceEndAsteriks from "../utils/sliceEndAsteriks"
...
const EditQuestion = () => {
  const { questionId, bookId, pageId } = useParams()
  const { bookPages } = useContext(BookPageApiData)
  const { authorBookList } = useContext(CreationApiData)
  const { processEditQuestion, questions } = useContext(QuestionApiData)
  ...
  useEffect(() => {
    let option = []
    let QuestionInfo = questions.filter(
      (item) => item.questionNo == questionId
    )[0]
    let questionOpt = QuestionInfo.options

    questionOpt.split("**").map((item, index) =>
      option.push({
        id: parseInt(index) + 1,
        value: item,
      })
    ...
    let newFormData = {
      ...
      questionTwo: QuestionInfo.question,
      answer: QuestionInfo.answer,
      questionNo: QuestionInfo.questionNo,
      imageUpload: QuestionInfo.images,
    }
  ...
  const handleOptionAssign = () => {
    let Option = ["None", "A", "B", "C", "D", "E", "F", "G", "H"]
    return Option
  }

  // formData.question.level.content
  ...
  const handleSubmit = () => {
    ...
    let newData = {
      ...
      question: formData.question?.level?.content || formData.questionTwo,
      options: SliceEndAsteriks(optionalAnsString.slice(0, -1)),
      imageUpload: formData.imageUpload ? formData.imageUpload : null,
      answer: formData.answer,
    }

    processEditQuestion(newData)
  }
