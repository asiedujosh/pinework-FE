...
const EditPage = () => {
  ...
  const { processEditBookPage, bookPages } = useContext(BookPageApiData)
  const { id } = useParams()
  ...
  useEffect(() => {
    let data = bookPages.filter((item) => item.id == id)[0]
    setFormData({
      pageNo: data.pageNo,
    ...
    <>
      <div className="w-90 m-6 md:mt-4 p-4 bg-white rounded shadow-lg">
        <div className="flex justify-center align-items mt-4">
          <h2 className="text-gray-600 text-xl font-semibold">{`Edit Page`}</h2>
        </div>
